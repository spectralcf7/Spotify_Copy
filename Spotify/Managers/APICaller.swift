        ...
        createRequest(with: URL(string: Constants.baseAPIURL + "/me/playlists/?limit=50"), type: .GET) { request in
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    ...
                    print("Playlists ABCD: \(result.items)")
            ...
            case .success(let profile):
                ...
                self?.createRequest(with: URL(string: urlString), type: .POST) { baseRequest in
                    ...
                    let task = URLSession.shared.dataTask(with: request) { data, _, error in
                        ...
                        do {
                            // We don't need to hand back a model - we only care about the response
    ...
    public func addTrackToPlaylist(track: AudioTrack, playlist: Playlist, completion: @escaping (Bool) -> Void) {
        createRequest(with: URL(string: Constants.baseAPIURL + "/playlists/\(playlist.id)/tracks"), type: .POST) { baseRequest in
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    let result = try JSONSerialization.jsonObject(with: data, options: .allowFragments)
                    if let response = result as? [String: Any], response["snapshot-id"] as? String != nil {
                        completion(true)
                    }
                    else {
                        completion(false)
